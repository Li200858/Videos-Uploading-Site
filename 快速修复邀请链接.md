...
3. 您的应用 URL 应该是：`https://videos-uploading-site.onrender.com`
   （如果 Render 自动添加了后缀，可能是：`https://videos-uploading-site-xxxx.onrender.com`）
...
4. 设置值为您的完整应用 URL：
   ```
   NEXTAUTH_URL=https://videos-uploading-site.onrender.com
   ```
   （如果您的实际 URL 不同，请使用实际的 URL）
...
手动替换为：
```
https://videos-uploading-site.onrender.com/login?email=xxx&token=xxx
```
...
```bash
# 在项目目录下运行
node scripts/create-test-invite.js <学生邮箱> <课程ID> https://videos-uploading-site.onrender.com
```
